...
router.get("/dashboard", requirePortalUser, async (req: Request, res: Response) => {
  try {
    const workspace = await requirePublisherWorkspace((req as any).portalUser.id);
    ...
    const eventsByBotId = new Map<string, typeof events>();
    ...
    const payloadBots = bots.map((bot) => {
      ...
      const metrics = summarizeBotMetrics(botEvents as any, bot as any);
...
router.get("/bots", requirePortalUser, async (req: Request, res: Response) => {
  try {
    const workspace = await requirePublisherWorkspace((req as any).portalUser.id);
    if (!workspace) return res.status(403).json({ error: "Bot developer workspace required" });
    ...
    return res.json({
      items: pageBots.map((bot) => ({
        ...
        sdkKeys: bot.sdkKeys.map((key) => toPublicSdkKey(key as any)),
...
router.post("/bots", requirePortalUser, async (req: Request, res: Response) => {
...
router.patch("/bots/:id", requirePortalUser, async (req: Request, res: Response) => {
...
router.delete("/bots/:id", requirePortalUser, async (req: Request, res: Response) => {
  try {
    const workspace = await requirePublisherWorkspace((req as any).portalUser.id);
    if (!workspace) return res.status(403).json({ error: "Bot developer workspace required" });
...
router.post("/bots/:id/keys", requirePortalUser, async (req: Request, res: Response) => {
...
router.get("/bots/:id/metrics", requirePortalUser, async (req: Request, res: Response) => {
  try {
    const workspace = await requirePublisherWorkspace((req as any).portalUser.id);
    if (!workspace) return res.status(403).json({ error: "Bot developer workspace required" });
    ...
    const metrics = summarizeBotMetrics(events as any, bot as any);
    return res.json({
