...
router.get("/dashboard", requirePortalUser, async (req: Request, res: Response) => {
  try {
    const workspace = await requireAdvertiserWorkspace((req as any).portalUser.id);
    ...
    const statsByCampaignId = new Map<string, { impressions: number; clicks: number; spendCents: number }>();
...
router.get("/campaigns", requirePortalUser, async (req: Request, res: Response) => {
  try {
    const workspace = await requireAdvertiserWorkspace((req as any).portalUser.id);
    if (!workspace) return res.status(403).json({ error: "Advertiser workspace required" });
...
router.post("/campaigns", requirePortalUser, async (req: Request, res: Response) => {
...
router.patch("/campaigns/:id", requirePortalUser, async (req: Request, res: Response) => {
...
router.delete("/campaigns/:id", requirePortalUser, async (req: Request, res: Response) => {
  try {
    const workspace = await requireAdvertiserWorkspace((req as any).portalUser.id);
    if (!workspace) return res.status(403).json({ error: "Advertiser workspace required" });
