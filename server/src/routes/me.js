...
router.post("/create-workspace", requirePortalUser, async (req, res) => {
...
router.get("/entry-context", requirePortalUser, async (req, res) => {
  try {
    const entry = await buildEntryContextByUserId(req.portalUser.id);
...
router.get("/organizations", requirePortalUser, async (req, res) => {
  try {
    const entry = await buildEntryContextByUserId(req.portalUser.id);
...
router.post("/default-workspace", requirePortalUser, async (req, res) => {
  ...
  try {
    const memberships = await prisma.organizationMember.findMany({
      where: { userId: req.portalUser.id, organizationId: parsed.data.workspaceId },
...
router.post("/switch-organization", requirePortalUser, async (req, res) => {
  const parsed = selectWorkspaceSchema.safeParse(req.body);
  if (!parsed.success) {
