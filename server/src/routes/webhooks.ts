...
import { enqueueWebhookProcess } from "../queue.js";
 ...
 */
export async function handleCaptureReversed(event: any): Promise<void> {
 ...
 */
export async function handlePayoutSucceeded(event: any): Promise<void> {
 ...
 */
export async function handlePayoutFailed(event: any): Promise<void> {
  const batchId = event.resource?.payout_batch_id || event.resource?.payout_item?.sender_item_id;
  if (!batchId) {
...
router.post("/paypal", async (req: Request, res: Response) => {
  ...
  try {
    await enqueueWebhookProcess(req.body);
  } catch (err) {
    logger.error("paypal-webhook: enqueue error", { eventType, err: String(err) });
