...
router.get("/balance", requirePortalUser, async (req: Request, res: Response) => {
...
router.post("/paypal/create-order", requirePortalUser, async (req: Request, res: Response) => {
  const cfg = await getPayPalConfig();
...
router.post("/paypal/capture-order", requirePortalUser, async (req: Request, res: Response) => {
  const cfg = await getPayPalConfig();
  if (!cfg.enabled) return res.status(503).json({ error: "PayPal not configured on this server" });
  ...
  try {
    ...
    const capture: any = await capturePayPalOrder(parsed.data.orderID);
    ...
    if (!amountCents) {
      return res.status(400).json({ error: "Could not determine payment amount" });
    }
...
router.post("/spend", requirePortalUser, async (req: Request, res: Response) => {
  const workspace = await requireAdvertiserWorkspace((req as any).portalUser.id);
  if (!workspace) return res.status(403).json({ error: "Advertiser workspace required" });
