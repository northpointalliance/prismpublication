...
export const verifySupabaseToken = async (token: string): Promise<{ id: string; email: string } | null> => {
...
export const validatePortalSession = async (req: Request, expectedEmail: string = ""): Promise<ValidationResult> => {
...
export const selectBestMembershipPerOrganization = (memberships: any[] = []): any[] => {
  const byOrganizationId = new Map<string, any>();
  ...
  for (const membership of memberships) {
    ...
    const orgId: string = membership.organization.id;
...
export const buildEntryContextByUserId = async (userId: string) => {
  ...
  const workspaces = memberships.flatMap((membership: any) => {
  ...
  const defaultWorkspaceId =
    user.defaultOrganizationId && workspaces.some((item: any) => item.orgId === user.defaultOrganizationId)
...
export const resolvePortalWorkspace = async (userId: string) => {
  ...
  const selectedMembership =
    memberships.find((membership: any) => membership.organizationId === user.defaultOrganizationId) || memberships[0];
...
export const readUserEmail = (req: Request): string => String(req.headers["x-user-email"] || "").trim().toLowerCase();

export const requirePortalUser = async (req: Request, res: Response, next: NextFunction) => {
...
export const requireAdminPortalUser = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const adminMembership = user?.memberships?.find(
    (m: any) => m.organization.type === "admin" && isAdminMembershipRole(m.role),
  );
  ...
  req.portalWorkspace = { user: user!, organization: adminMembership.organization, membership: adminMembership };
  return next();
};
...
export const requireSdkKey = async (req: Request, res: Response, next: NextFunction) => {
  const token = getBearerToken(req.headers.authorization);
  if (!token) {
...
export const ensureSdkBotScope = (req: Request, botId: string): { ok: boolean; status?: number; error?: string } => {
...
export const requireSdkSignature = (req: Request, res: Response, next: NextFunction) => {
